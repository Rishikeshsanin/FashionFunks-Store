...
export function CheckoutPage() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const history = JSON.parse(window.localStorage.getItem(ORDER_HISTORY_KEY) ?? "[]") as OrderRecord[];
  ...
  if (!cart.length) return <div className="page-shell checkout-empty"><span className="eyebrow">Nothing to check out</span><h1>Your bag is empty.</h1><p>Add a piece before placing an order.</p><Link className="button button--primary" href="/shop">Shop new arrivals</Link></div>;

  return (
    <div className="checkout-page">
      <div className="container checkout-page__head"><Link href="/cart">← Back to bag</Link><span>Secure checkout</span></div>
      <div className="container checkout-grid">
        <form className="checkout-form" onSubmit={submit}>
          <span className="eyebrow">Almost yours</span><h1>Place your order</h1><p className="checkout-form__intro">Review your pieces and place your order in one simple step.</p>
          ...
          <div className="checkout-step checkout-step--last"><span><CheckIcon /></span><div><h2>No card details needed</h2><p>Your order confirmation is created instantly when you continue.</p></div></div>
          <label className="confirm-check"><input type="checkbox" required /><span>I’ve reviewed my order and I’m ready to place it.</span></label>
          <button className="button button--primary button--wide button--large" type="submit" disabled={submitting}>{submitting ? "Placing your order…" : `Place order · ${formatMoney(totals.total)}`}</button>
        </form>
