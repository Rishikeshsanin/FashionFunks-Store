    ...
    return (
      <div className="account-page container">
        ...
        <div className="account-grid">
          <section><h2>Order history</h2>{orders.length ? <div className="order-history">{orders.map((order) => <article key={order.id}><div><strong>{order.id}</strong><span>{new Intl.DateTimeFormat("en-IN", { dateStyle: "medium" }).format(new Date(order.createdAt))}</span></div><div><span>{order.itemCount} piece{order.itemCount === 1 ? "" : "s"}</span><strong>{formatMoney(order.total)}</strong></div></article>)}</div> : <div className="account-empty"><p>No orders yet. Anything you place while logged in will appear here.</p><Link className="arrow-link" href="/shop">Explore the collection →</Link></div>}</section>
          <aside><h2>Account details</h2><dl><div><dt>Name</dt><dd>{user.name}</dd></div><div><dt>Email</dt><dd>{user.email}</dd></div><div><dt>Account type</dt><dd>FashionFunks member</dd></div></dl><p>Your account details and order history are securely saved on this device.</p></aside>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="auth-page">
      ...
      <section className="auth-card">
        ...
        <p>{mode === "login" ? "Log in to keep your orders and wishlist together." : "Create your FashionFunks account. No phone number, no noise."}</p>
        ...
        <form onSubmit={submit}>
          ...
          <label className="form-field"><span>Email</span><input value={email} onChange={(event) => setEmail(event.target.value)} type="email" autoComplete="email" placeholder="you@example.com" required /><small>We don’t spam. Your details stay securely on this device.</small></label>
          ...
          <button className="button button--primary button--wide" type="submit">{mode === "login" ? "Log in" : "Create account"}</button>
        </form>
